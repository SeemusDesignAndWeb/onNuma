<script>
	...
	function isEmailAlreadySignedUp(rota, occurrenceId) {
		if (!email && !matchedContact) return false;
		const assignees = getAssigneesForRotaOccurrence(rota, occurrenceId);
		
		const emailsToCheck = new Set();
		if (email) emailsToCheck.add(email.toLowerCase().trim());
		if (matchedContact?.email) emailsToCheck.add(matchedContact.email.toLowerCase().trim());
		if (signUpWithSpouse && spouse?.email) emailsToCheck.add(spouse.email.toLowerCase().trim());
