/**
 * Marketing seed data – creates professional onboarding email templates,
 * content blocks, links, and sequences for TheHUB platform.
 */

import { randomUUID } from 'crypto';
import { create, readCollection } from './fileStore.js';
...
const stepsList = (steps) => {
	...
	for (const s of steps) {
		html += `<li style="margin:8px 0;font-size:14px;color:#272838;line-height:1.6;">${s}</li>`;
	}
	html += '</ol>';
	return html;
};
...
/**
 * Seeds all marketing content: blocks, links, templates, and sequences.
 * Skips if content already exists (checks by name/key).
 * @param {string} adminId – the multi-org admin ID who is creating this
 * @returns {Promise<{ blocks: number, links: number, templates: number, sequences: number, steps: number }>}
 */
export async function seedMarketingContent(adminId) {
	const counts = { blocks: 0, links: 0, templates: 0, sequences: 0, steps: 0 };
	...
	for (const block of getContentBlocks()) {
		if (existingBlockKeys.has(block.key)) continue;
		await create('marketing_content_blocks', {
			id: id(),
			title: block.title,
			key: block.key,
	...
	for (const link of getLinks()) {
		if (existingLinkKeys.has(link.key)) continue;
	...
	for (const tmpl of getEmailTemplates()) {
		if (existingTemplateNames.has(tmpl.name)) {
			// Find the existing one for sequence steps
		...
		await create('marketing_email_templates', {
			id: tid,
			name: tmpl.name,
		...
		// Version
		await create('marketing_template_versions', {
			id: id(),
			template_id: tid,
			...
			updated_at: now(),
			updated_by: adminId,
			change_summary: 'Initial creation (seeded)'
		});
	}
	...
	if (!existingSequences.some((s) => s.name === seqName)) {
		const seqId = id();
		await create('marketing_sequences', {
			id: seqId,
			name: seqName,
		...
		const steps = [
			...
			{ name: 'Tips for Success with TheHUB', delay: 21, unit: 'days', conditions: [{ type: 'not_logged_in_since_joining', value: '' }] }
		];
		...
		for (let i = 0; i < steps.length; i++) {
			...
			await create('marketing_sequence_steps', {
				...
				order: i + 1,
				delay_value: step.delay,
				delay_unit: step.unit,
