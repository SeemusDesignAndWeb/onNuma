...
<svelte:head>
	<title>My schedules – MyHUB</title>
</svelte:head>

<div class="my-page-content">
	<h1 class="my-heading">My schedules</h1>
		...
		<div class="my-card">
			<div class="my-card-body">
				<div class="my-alert my-alert-info" role="status">
					<p>You don't have any upcoming schedules yet.</p>
					<p class="mt-4">
						<a href="/myhub/opportunities" class="my-link">Sign up for schedules here</a> if you'd like to volunteer.
					</p>
				</div>
			</div>
		</div>
	{:else}
		<p class="my-lead">
			{#if memberName}Showing schedules for <strong>{memberName}</strong>. {:else}Your upcoming schedules.{/if}
		...
		<div class="my-card">
			<div class="my-card-body">
				<ul class="my-rota-list" role="list">
					{#each sortedRotas as rota}
						...
						<li class="my-rota-item">
							...
							{#if rota.teamName}
								<span class="my-rota-team">{rota.teamName}</span>
							{/if}
							...
							<div class="my-rota-cannot-attend">
								{#if confirmingRotaKey === confirmKey}
									<form method="POST" action="?/cannotVolunteer" class="my-cannot-form">
										...
										<p class="my-cannot-confirm-text">We'll email the schedule owner so they can find a replacement. Are you sure?</p>
				...
				<p class="mt-6 text-[1.0625rem] text-gray-600">
					Want to do more?
					<a href="/myhub/opportunities" class="my-btn my-btn-primary my-rota-signup-btn">Sign up for more schedules</a>
				</p>
			</div>
		</div>
	{/if}
