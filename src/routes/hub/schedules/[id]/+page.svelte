<script>
	...
	$: pastRoleContactIds = $page.data?.pastRoleContactIds || [];
	...
	async function handleDelete() {
		const confirmed = await dialog.confirm('Are you sure you want to delete this schedule?', 'Delete Schedule');
		if (confirmed) {
	...
	$: pastRoleContactIdSet = new Set(pastRoleContactIds);

	$: filteredAvailableContacts = (() => {
		let base = contactsFilteredByList;
		if (includePastRole && pastRoleContactIds.length > 0) {
			base = base.filter(c => pastRoleContactIdSet.has(c.id));
		}
					...
					if (conflicts && conflicts.length > 0) {
						...
						conflicts.forEach(c => {
							if (c.type === 'holiday') {
								...
								conflictMessage += `- ${c.contactName}: ${c.scheduleRole} for ${c.eventName}\n`;
							}
						});
	...
	async function handleAddHelpFile() {
		if (helpFileType === 'link') {
			...
			try {
				...
				const uploadResponse = await fetch(`/hub/schedules/${rota.id}/help-files/upload`, {
					method: 'POST',
					body: uploadFormData
				});
...
{#if rota}
	<div class="hub-top-panel p-6 mb-6">
		<div class="flex flex-col sm:flex-row sm:justify-between sm:items-center gap-4 mb-6">
			<h2 class="text-xl sm:text-2xl font-bold text-gray-900">{rota.role || 'Unknown'} {$terminology.rota.toLowerCase()}</h2>
	...
	<div class="bg-white shadow rounded-lg p-6">
		...
		{:else}
			<p class="text-gray-500">No assignees yet. Click "Add Assignees" to assign people to this schedule.</p>
		{/if}
	</div>
		...
		<div class="fixed inset-0 bg-black bg-opacity-50 flex items-center justify-center z-50" role="button" tabindex="0" on:click={() => { showAddAssignees = false; searchTerm = ''; guestName = ''; selectedContactIds = new Set(); selectedListId = ''; }} on:keydown={(e) => e.key === 'Escape' && (showAddAssignees = false)} aria-label="Close modal">
			<div class="bg-white rounded-lg max-w-2xl w-full max-h-[80vh] flex flex-col" on:click|stopPropagation role="dialog" aria-modal="true">
				...
				<div class="p-6 border-b border-gray-200">
					...
					{#if pastRoleContactIds.length > 0}
						<label class="flex items-center gap-2 mt-3 text-sm text-gray-600 cursor-pointer">
							...
							Include people who've served in this role elsewhere ({pastRoleContactIds.length})
						</label>
					{/if}
				</div>
