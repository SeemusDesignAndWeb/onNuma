...
export async function load({ params, cookies, url, parent }) {
	...
	if (!rota) {
		throw redirect(302, '/hub/schedules');
	}
	...
	try {
		...
		signupLink = `${baseUrl}/signup/schedule/${token.token}`;
	} catch (error) {
	...
	const pastRoleContactIds = await findContactsByPastRole(rota.role, rota.id, organisationId);
...
export const actions = {
	update: async ({ request, params, cookies, url, locals }) => {
		...
		try {
			...
			return fail(400, { error: error.message || 'Failed to update schedule' });
		}
	},
		...
		inviteToMyhub: async ({ request, params, cookies, url }) => {
			...
			try {
				const organisationId = await getCurrentOrganisationId();
				const rota = await findById('rotas', params.id);
				if (!rota) return fail(404, { error: 'Schedule not found.', type: 'inviteToMyhub' });
