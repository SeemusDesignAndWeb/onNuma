<script lang="js">
	...
	async function deleteImage(id) {
		...
		try {
			const response = await fetch(`/hub/api/images?id=${id}`, {
				method: 'DELETE'
			});
...
<svelte:head>
	<title>Manage Images - TheHUB</title>
</svelte:head>
...
<div class="container mx-auto px-4 py-8">
	...
	<div class="bg-white p-6 rounded-lg shadow mb-6">
		...
		<div class="space-y-4">
			<div>
				<label for="image-upload" class="block text-sm font-medium mb-2">Select Image</label>
				<input
					id="image-upload"
					type="file"
					accept="image/*"
				...
				<p class="text-sm text-gray-500 mt-1">
					Supported formats: JPG, PNG, GIF, WebP. Max size: 10MB. Images are stored in /images (volume or static/images locally).
				</p>
			</div>
			{#if uploadError}
