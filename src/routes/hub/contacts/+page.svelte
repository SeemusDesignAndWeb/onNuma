...
<div class="mb-4 flex flex-col sm:flex-row sm:justify-between sm:items-center gap-4">
	<h2 class="text-xl sm:text-2xl font-bold">{volunteersLabel}</h2>
...
<div class="mb-4">
	<form on:submit|preventDefault={handleSearch} class="flex gap-2">
		<input
			...
			placeholder="Search {volunteersLabel.toLowerCase()}..."
...
{#if overPlanLimit}
	<p class="mb-4 text-sm text-amber-800 bg-amber-50 border border-amber-200 rounded-md px-3 py-2">
		Your plan includes access to the first {planLimit} {volunteersLabel.toLowerCase()} (sorted by name). You have {totalInOrg} in total; the rest are not shown but are kept. Upgrade your plan to see and manage all {volunteersLabel.toLowerCase()}.
	</p>
{/if}
...
{#if bulkUpdateResult}
	<div class="mb-4 p-4 rounded-md {bulkUpdateResult.error ? 'bg-red-50 border border-red-200' : 'bg-green-50 border border-green-200'}">
		...
		{:else}
			<p class="text-green-800">{bulkUpdateResult.message || `Successfully updated ${bulkUpdateResult.updatedCount || 0} ${volunteerLabel}(s).`}</p>
		{/if}
	</div>
{/if}

<div role="region" aria-label="{volunteersLabel} table">
	<Table {columns} {mobileColumns} rows={contacts} emptyMessage="No {volunteersLabel.toLowerCase()} yet. Add your first {volunteerLabel.toLowerCase()} above." onRowClick={(row) => goto(`/hub/contacts/${row.id}`)} />
</div>
...
{#if showBulkUpdateDialog}
	...
	>
		<div class="bg-white rounded-lg p-6 max-w-lg w-full mx-4 max-h-[90vh] overflow-y-auto" on:click|stopPropagation role="dialog" aria-labelledby="bulk-update-title" aria-modal="true">
			<h3 id="bulk-update-title" class="text-xl font-bold text-gray-900 mb-4">Bulk Update {volunteersLabel}</h3>
			...
			>
				...
				<div class="space-y-4 mb-6">
					...
					<!-- Filter Condition -->
					<div>
						<label class="block text-sm font-medium text-gray-700 mb-2">Update Which {volunteersLabel}?</label>
						<select 
				...
				<div class="flex gap-3 justify-end">
					...
					<button
						...
						disabled={isSubmitting}
					>
						{isSubmitting ? 'Updating...' : `Update ${volunteersLabel}`}
					</button>
				</div>
			</form>
		</div>
