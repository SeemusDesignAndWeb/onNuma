<script>
    ...
    $: editHref = $page.url.pathname + '?edit=true' + (highlightOccurrenceId ? '&occurrenceId=' + highlightOccurrenceId : '');

    let occurrenceLinkCopied = {};
    let showOptionsPanel = true;
    ...
    $: if (formResult && browser && formResult !== lastProcessedFormResult) {
        ...
        if (formResult?.emailId) {
            createdEmailId = formResult.emailId;
...
{#if event}
    <div class="bg-white shadow rounded-lg p-3 sm:p-4 mb-4">
            ...
            {#if viewingOccurrence}
                <div class="flex flex-wrap items-center gap-2 sm:gap-3">
                    <span class="text-sm text-gray-600">
                        <span class="font-normal">Currently viewing:</span>
                        <span class="font-semibold text-gray-800">{formatDateTimeUK(viewingOccurrence.startsAt)}</span>
                    </span>
                    <a
        ...
        <!-- Two-column layout: content (left, expanded) + Options sidebar (right) -->
        <div class="grid grid-cols-1 lg:grid-cols-5 gap-4 mt-4">
            <!-- Left: form or read-only content - more space for description -->
        ...
        {#if editing}
            <form id="event-edit-form" method="POST" action="?/update" use:enhance>
                ...
                <div class="grid grid-cols-1 lg:grid-cols-4 gap-3 sm:gap-4">
                    <!-- Basic Information - Narrow Column -->
                    ...
                    <!-- Description & Options & Email - Wide Column -->
                    <div class="lg:col-span-3 space-y-4">
                        ...
                        <!-- Collapsible Options panel (light blue) -->
                        <div class="bg-hub-blue-50 border border-hub-blue-200 rounded-lg overflow-hidden">
                            <button
                                type="button"
                                class="w-full flex items-center justify-between px-4 py-3 text-left text-sm font-semibold text-gray-900 hover:bg-hub-blue-100/50"
                                on:click={() => showOptionsPanel = !showOptionsPanel}
                                aria-expanded={showOptionsPanel}
                            >
                                <span>Options</span>
                                <svg class="w-5 h-5 text-gray-500 transition-transform" class:rotate-180={!showOptionsPanel} fill="none" stroke="currentColor" viewBox="0 0 24 24">
                                    <path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M19 9l-7 7-7-7" />
                                </svg>
                            </button>
                            {#if showOptionsPanel}
                                <div class="px-4 pb-4 pt-0 space-y-4 border-t border-hub-blue-200">
                                    <div>
                                    ...
                                    <div class="flex items-center">
                                        <input type="checkbox" id="enableSignup" name="enableSignup" bind:checked={formData.enableSignup} class="h-4 w-4 text-hub-green-600 focus:ring-theme-button-2 border-gray-300 rounded" />
                                        <label for="enableSignup" class="ml-2 block text-xs text-gray-700">Enable signup</label>
                                    </div>
                                    <div class="flex items-center">
                                        <input type="checkbox" id="hideFromEmail" name="hideFromEmail" bind:checked={formData.hideFromEmail} class="h-4 w-4 text-hub-green-600 focus:ring-theme-button-2 border-gray-300 rounded" />
                                        <label for="hideFromEmail" class="ml-2 block text-xs text-gray-700">Hide from email</label>
                                    </div>
                                    {#if formData.enableSignup}
                                        <div class="flex items-center">
                                            <input type="checkbox" id="showDietaryRequirements" name="showDietaryRequirements" bind:checked={formData.showDietaryRequirements} class="h-4 w-4 text-hub-green-600 focus:ring-theme-button-2 border-gray-300 rounded" />
                                            <label for="showDietaryRequirements" class="ml-2 block text-xs text-gray-700">Show dietary requirements on signup</label>
                                        </div>
                                        <div>
                                            <label for="maxSpaces" class="block text-xs font-medium text-gray-700 mb-1">Max spaces</label>
                                            <input id="maxSpaces" name="maxSpaces" type="number" min="1" bind:value={formData.maxSpaces} class="w-20 rounded-md border border-gray-300 shadow-sm focus:border-theme-button-2 focus:ring-theme-button-2 py-1.5 px-2 text-sm" />
                                        </div>
                                    {/if}
                                    <div>
                                        ...
                                        <div class="flex items-center gap-2 mt-1">
                                            ...
                                            <select id="event-color" name="color" bind:value={formData.color} class="flex-1 rounded-md border border-gray-300 shadow-sm focus:border-theme-button-2 focus:ring-theme-button-2 py-2 px-2 text-sm">
                        ...
                        <div class="bg-gray-50 rounded-lg p-3">
                            ...
                            <div>
                                ...
                                />
                                <p class="text-xs text-gray-500 mt-1">Email list(s) to include this event on. If none selected, event is sent to everyone (by visibility).</p>
                            </div>
                        </div>
                        ...
                        <div class="bg-gray-50 rounded-lg p-3">
                            ...
                            <div class="flex flex-wrap items-center gap-4">
                                <div>
                                    ...
                                    {#if formData.image}
                                        <div class="mb-2 rounded border border-gray-300 overflow-hidden bg-gray-100 aspect-[2/1] min-h-[80px] w-40">
                                            <img src={formData.image} alt="Event preview" class="w-full h-full object-cover" />
                                        </div>
                                    {/if}
                                    <button type="button" on:click={() => showImagePicker = true} class="text-xs py-2 px-3 rounded-md border border-gray-500 bg-white text-gray-700 hover:bg-gray-50">
                    ...
                    <div class="border-t border-gray-200 pt-4">
                        ...
                        <div class="border-t border-gray-200 pt-4">
                            ...
                            <dl class="space-y-3">
                                ...
                                <div>
                                    <dt class="text-xs font-medium text-gray-500 uppercase">Email list</dt>
        ...
        {/if}
            ...
            <!-- Options sidebar - panel same width as buttons (3/4 of column) -->
            <aside class="lg:col-span-1 flex justify-center lg:justify-end">
                <div class="sticky top-4 w-full bg-gray-50 rounded-lg p-4 flex flex-col gap-3">
                    <h3 class="text-xs font-semibold text-gray-900 w-full">Options</h3>
                    {#if editing}
                        <button type="submit" form="event-edit-form" class="w-full inline-flex items-center justify-center gap-1 px-2.5 py-1.5 rounded-md bg-hub-green-600 hover:bg-hub-green-700 text-white text-xs font-medium">
                            <svg class="w-3.5 h-3.5" fill="none" stroke="currentColor" viewBox="0 0 24 24"><path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M5 13l4 4L19 7" /></svg>
                            Save Changes
                        </button>
                        <button type="button" on:click={handleDelete} class="w-full inline-flex items-center justify-center gap-1 px-2.5 py-1.5 rounded-md bg-hub-red-600 hover:bg-hub-red-700 text-white text-xs font-medium">
                            <svg class="w-3.5 h-3.5" fill="none" stroke="currentColor" viewBox="0 0 24 24"><path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M19 7l-.867 12.142A2 2 0 0116.138 21H7.862a2 2 0 01-1.995-1.858L5 7m5 4v6m4-6v6m1-10V4a1 1 0 00-1-1h-4a1 1 0 00-1 1v3M4 7h16" /></svg>
                            Delete
                        </button>
                    {:else}
                        <a href={editHref} class="w-full inline-flex items-center justify-center gap-1 px-2.5 py-1.5 rounded-md bg-hub-green-600 hover:bg-hub-green-700 text-white text-xs font-medium">
                    ...
                    {/if}
                    <a href="/hub/events/calendar" class="w-full inline-flex items-center justify-center gap-1 px-2.5 py-1.5 rounded-md border border-hub-blue-500 bg-white text-hub-blue-600 hover:bg-hub-blue-50 text-xs font-medium">
            ...
            <div transition:slide={{ duration: 200 }}>
                <div class="mt-3 pt-3 border-t border-gray-200">
                    <Table
                        ...
                        rows={occurrences}
                        onRowClick={(row) => goto(`/hub/events/${event.id}/occurrences/${row.id}?edit=true`)}
