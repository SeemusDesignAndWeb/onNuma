<script lang="js">
	...
	async function saveHome() {
		...
		try {
			...
			if (response.ok) {
				notifications.success('Home page settings saved successfully!');
			} else {
				const error = await response.json();
				notifications.error(error.error || 'Failed to save home page');
			}
		} catch (error) {
			console.error('Failed to save home page:', error);
			notifications.error('Failed to save home page');
		} finally {
			saving = false;
		}
	}
