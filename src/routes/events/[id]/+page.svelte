...
{#if data.event.image}
	<section
		class="relative h-[40vh] overflow-hidden"
		style="background-image: url('{data.event.image}'); background-size: cover; background-position: center;"
	>
		<div class="absolute inset-0 bg-black bg-opacity-50"></div>
		<div class="relative h-full flex items-center">
			<div class="container mx-auto px-4">
				<h1 class="text-white text-4xl md:text-5xl font-bold">{data.event.title}</h1>
			</div>
		</div>
	</section>
{:else}
