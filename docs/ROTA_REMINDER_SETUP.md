# Schedule Reminder Notifications Setup Guide
...
**Configuring “days before event”:** In the Hub go to **Settings → Advanced → Schedule reminder emails**. Set “Days before event to send reminder” (0–90). The cron job uses this value when you call the endpoint without a `daysAhead` query parameter.
...
SCHEDULE_REMINDER_CRON_SECRET=your-generated-secret-token-here
```
...
Copy the output and use it as your `SCHEDULE_REMINDER_CRON_SECRET` value.
...
The **Hub setting** (Settings → Advanced → Schedule reminder emails) overrides this when the cron is called without a `daysAhead` parameter.
...
2. **Create a new cron job** with these settings:
   - **URL**: `https://your-domain.com/api/cron/schedule-reminders?secret=YOUR_SECRET_TOKEN`
...
3. **Test the endpoint** first:
   ```bash
   curl "https://your-domain.com/api/cron/schedule-reminders?secret=YOUR_SECRET_TOKEN"
   ```

4. **Save and activate** the cron job

**Example cron-job.org setup:**
- URL: `https://yourdomain.com/api/cron/schedule-reminders?secret=abc123xyz...`
...
3. Configure:
   - **Schedule**: `0 9 * * *` (9 AM daily)
   - **Command**: `curl "https://your-domain.com/api/cron/schedule-reminders?secret=$SCHEDULE_REMINDER_CRON_SECRET"`
...
{
  "success": true,
  "message": "Schedule reminders processed for 3 days ahead",
  "results": {
    "totalContacts": 5,
    "totalAssignments": 7,
    "sent": 7,
...
```bash
curl "https://your-domain.com/api/cron/schedule-reminders?secret=YOUR_SECRET&daysAhead=1"
```
...
curl "https://your-domain.com/api/cron/schedule-reminders?secret=wrong-secret"
```
...
2. **Check your application logs** (Railway logs) for:
   ```
   [Schedule Reminders API] Starting reminder job for 3 days ahead
   [Schedule Reminders] Sent reminder to contact@example.com for Event Name - Role Name
   [Schedule Reminders API] Reminder job completed: 5 sent, 0 failed
   ```
...
**Test manually:**
```bash
curl "https://your-domain.com/api/cron/schedule-reminders?secret=YOUR_SECRET"
```
...
**Solution**:
1. Verify `SCHEDULE_REMINDER_CRON_SECRET` is set in your environment variables
...
2. Change `SCHEDULE_REMINDER_DAYS_AHEAD` environment variable
...
4. **Secure Secret**: Keep `SCHEDULE_REMINDER_CRON_SECRET` secure and never commit it to git
...
1. **Environment variable** (applies to all requests):
   ```env
   SCHEDULE_REMINDER_DAYS_AHEAD=7
   ```

2. **Query parameter** (per request):
   ```
   https://your-domain.com/api/cron/schedule-reminders?secret=YOUR_SECRET&daysAhead=7
   ```
...
- The endpoint is protected by the `SCHEDULE_REMINDER_CRON_SECRET` token
...
- [ ] Generated `SCHEDULE_REMINDER_CRON_SECRET` with `openssl rand -base64 32`
- [ ] Set `SCHEDULE_REMINDER_CRON_SECRET` in environment variables (local and production)
- [ ] Optionally set `SCHEDULE_REMINDER_DAYS_AHEAD` (defaults to 3)
...
Once all items are checked, your schedule reminder system is fully operational!
